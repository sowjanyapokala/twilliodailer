...
post '/dial' do
    ...
    response = Twilio::TwiML::Response.new do |r|
       ...
       # r.Dial :callerId => dial_id do |d|
        r.Dial(:record=>"record-from-answer", :callerId => dial_id, :method => "GET", :action=>"http://yardidhruv-touchpoint.cs62.force.com/Click2Dial/TwilioCalls/TouchPoint?ToNumber=#{number}")  do |d|
          d.Number number
        end
    end
    puts response.text
